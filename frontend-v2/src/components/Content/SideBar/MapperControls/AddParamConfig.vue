<script lang="ts" setup>
  ...
  function addNewParamConfig() {
    ...
    if (intervals.value === '' || overlap.value === '') {
      return;
    }
...
<template>
  <button @click="toggleModal">Add Param Config</button>
  ...
  <div v-show="showModal" class="fixed z-10 overflow-y-auto top-0 w-full left-0" id="modal">
    <div class="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      >
        <div class="bg-white p-4 grid grid-cols-3">
          <label class="col-span-1">Intervals</label>
          <input class="col-span-2" type="text" v-model="intervals" placeholder="Enter integer greater than 0" />
        </div>
        <div class="bg-white p-4 grid grid-cols-3">
          <label class="col-span-1">Overlap %</label>
          <input class="col-span-2" type="text" v-model="overlap" placeholder="Enter integer in range [0, 99]" />
        </div>
